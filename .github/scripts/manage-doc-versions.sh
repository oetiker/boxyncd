...
update_versions_json() {
    ...
    for v in $versions; do
        ...
        json_versions+="{\"version\":\"$v\",\"path\":\"/boxyncd/$v/\"}"
        first=false
    done
...
<head>
    <meta charset="utf-8">
    <title>boxyncd Documentation</title>
    <script>
        // Redirect to latest stable version
        fetch('/boxyncd/versions.json')
            .then(r => r.json())
            .then(data => {
                const target = data.latest ? `/boxyncd/${data.latest}/` : '/boxyncd/dev/';
                window.location.replace(target);
            })
            .catch(() => {
                // Fallback redirect
EOF
    echo "                window.location.replace('/boxyncd/${latest}/');" >> "$SITE_DIR/index.html"
    cat >> "$SITE_DIR/index.html" << 'EOF'
            });
    </script>
    <noscript>
EOF
    echo "        <meta http-equiv=\"refresh\" content=\"0; url=/boxyncd/${latest}/\">" >> "$SITE_DIR/index.html"
    cat >> "$SITE_DIR/index.html" << 'EOF'
    </noscript>
</head>
<body>
